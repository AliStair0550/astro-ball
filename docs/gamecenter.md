...
**The images are in `docs/achievements/`**, one per id, 512x512 RGB with
no alpha, which is what App Store Connect requires before it will save an
achievement's localization. They are generated by
`tools/make_achievements.py` in the game's own language — the comet, the
bricks with their v2 bevel, the blast brick's burst, and for
CONSTELLATION CHARTED and FULL CHART the actual figure from the star
chart, drawn from the same coordinates the game uses.

Re-run the tool to change them; do not edit the PNGs. A picture nobody
can regenerate is a picture nobody changes.
