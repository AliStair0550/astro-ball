...
## score er, gridet ved ikke, at der findes power-ups, og baggrunden ved
## kun, at noget skete et bestemt sted. Alt mødes her.

enum State { PLAYING, BALL_LOST, LEVEL_CLEAR, GAME_OVER }

const START_LIVES := 3
const MAX_BALLS := 6
const BALL_LOST_PAUSE := 0.4
const LEVEL_CLEAR_PAUSE := 2.0
const GAME_OVER_PAUSE := 2.2
...
func _ready() -> void:
	...
	if level_paths.is_empty():
		push_error("Ingen levels i res://levels")
		return
	_load_level(0)
...
func _on_wall_hit(pos: Vector2, _normal: Vector2) -> void:
	...
	game_feel.shake(SHAKE_WALL.x, SHAKE_WALL.y)
...
func _on_ball_lost(ball: Ball) -> void:
	...
	state = State.GAME_OVER if lives <= 0 else State.BALL_LOST
	_state_timer = GAME_OVER_PAUSE if lives <= 0 else BALL_LOST_PAUSE
	_refresh_hud()
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo:
		match event.keycode:
			...
			KEY_F2:
				# Hop til næste level. Kun til at teste med.
				_next_level()
			KEY_F3:
				_load_level(level_index)
...
func _update_debug() -> void:
	...
	var lines := [
		...
		"KLODSER    %d tilbage" % grid.remaining_breakable(),
		"BOLDE      %d" % _balls.size(),
		"KOMBO      %d (bedste %d, x%d)" % [combo, best_combo, _combo_multiplier()],
		"LIV        %d" % lives,
	]
	if ball:
	...
	lines.append("AKTIVE     %s" % ("ingen" if names.is_empty() else ", ".join(names)))
	lines.append("F1 skjul · F2 næste level · F3 genstart")
	_debug_label.text = "\n".join(lines)
